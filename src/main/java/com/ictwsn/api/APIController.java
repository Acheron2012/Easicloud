...
public class APIController {
	...
	public String GPPrediction(HttpServletRequest request, HttpServletResponse response, @PathVariable int sensor_id,
		...
		for (int i = 0; i < res.length; i++) {
			...
			res[i] = result[0].get(0, 0)+10;//remove
	...
	@RequestMapping(value = "/sensor/{sensor_id}/datapoints/MIMOGPPrediction", method = RequestMethod.GET)
	public String MIMOGPPrediction(HttpServletRequest request, HttpServletResponse response, @PathVariable int sensor_id,
			@RequestParam(value = "start", required = false) String start,
			@RequestParam(value = "end", required = false) String end,
			@RequestParam(value = "data_type", required = false) String data_type,
			@RequestParam(value = "exponent_number", required = false) String exponent_number,
			@RequestParam(value = "outputNumber", required = false) String outputNumber,
	        @RequestParam(value = "horizon", required = false) String horizon,
	        @RequestParam(value = "predict_alg_type", required = false) String predict_alg_type){
		System.out.println(start);
		System.out.println(end);
		System.out.println(data_type);
		System.out.println(exponent_number);
		System.out.println(outputNumber);
		System.out.println(horizon);
		System.out.println(predict_alg_type);
		// 判断用户id
		int user_id = getUserId(request);
		if (user_id == -1) {
			logger.error("user_key不正确_：{}", request.getHeader("user_key"));
		...
		Object[] res = null;
		try {
			res = m.MIMOGaussian(1, nums,4,1,10);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		System.out.println(outputNumber);
		double[]result=new double[Integer.valueOf(horizon)];
		int alg=Integer.valueOf(predict_alg_type);
		switch(alg){
		case 1:
			result=algorithmAPI.getMimoGaussianProcess(nums, Integer.valueOf(exponent_number), Integer.valueOf(outputNumber),Integer.valueOf(horizon));
			break;
		case 2:
			result=algorithmAPI.getGaussianProcess(nums, Integer.valueOf(exponent_number), Integer.valueOf(horizon));
			break;
		case 3:
			break;
		}
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("history", jsonArray);
		jsonObject.put("predict_value", result);
		System.out.println(jsonObject.toString());
		responseToJSON(response, jsonObject.toString());
		return null;
	}
	...
	public String CorrelationAnalysis(HttpServletRequest request, HttpServletResponse response,
		...
		datatype_numbers=data_type.size();
		double[][] pearsons=new double[datatype_numbers][datatype_numbers];
		double[][] spearmans=new double[datatype_numbers][datatype_numbers];
		for(int i=0;i<datatype_numbers;i++){
			for(int j=0;j<datatype_numbers;j++){
				double[] nums1 = new double[jsonArray.size()];
				
				double[] nums2 = new double[jsonArray.size()];
				for (int m = 0; m < jsonArray.size(); m++) {
					JSONArray jsonArray2 = jsonArray.getJSONObject(m).getJSONArray("values");
					nums1[m] = jsonArray2.getJSONObject(0).getDouble(data_type.get(i));
					nums2[m] = jsonArray2.getJSONObject(0).getDouble(data_type.get(j));
					//System.out.println(nums2[m]);

				}
				pearsons[i][j]=algorithmAPI.getPearsonsCorrelation(nums1, nums2);
				spearmans[i][j]=algorithmAPI.getSpearmansCorrelation(nums1, nums2);
			}
		}
		JSONObject jsonObject2 = new JSONObject();
		jsonObject2.put("data_type", data_type);
		jsonObject2.put("pearsons", pearsons);
		jsonObject2.put("spearmans", spearmans);
		System.out.println(jsonObject2.toString());
		responseToJSON(response, jsonObject2.toString());
		return null;
	}
