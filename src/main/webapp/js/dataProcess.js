...
function prediction() {
	...
	var para = {
		...
		data_type : $("#data_type").val(),
		predict_alg_type:$("#select_predict_algorithm").val(),
		exponent_number : $("#exponent_number").val(),
		outputNumber : $("#outputNumber").val(),
		horizon : $("#horizon").val()
	};
	$.ajax({
		url : "v1/sensor/" + sensor_id + "/datapoints/MIMOGPPrediction",// 跳转到
		// action
		data : para,
		type : 'get',
		cache : false,
...
function getCorrelationResult() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			var pearsons=data.pearsons;
			var spearmans=data.spearmans;
			$("#myTable1").empty();
			$("#myTable2").empty();
			var a = document.getElementById("myTable1");
			var x=document.getElementById("myTable1").createCaption();
			
			x.innerHTML="<b>Pearsons相关系数</b>";
			var b = document.getElementById("myTable2");
			var y=document.getElementById("myTable2").createCaption();
			y.innerHTML="<b>Spearmans相关系数</b>";
			for(var i=0;i<data_type.length+1;i++){
				var row=a.insertRow(i);
				var row2=b.insertRow(i);
				for(var j=0;j<data_type.length+1;j++){
					var col=row.insertCell(j);
					var col2=row2.insertCell(j);
					if(i==0&&j==0){
						col.innerHTML="";
						col2.innerHTML="";
					}else if(i==0){
						col.innerHTML=data_type[j-1];
						col2.innerHTML=data_type[j-1];
					}else if(j==0){
						col.innerHTML=data_type[i-1];
						col2.innerHTML=data_type[i-1];
					}else{
						col.innerHTML=pearsons[i-1][j-1].toFixed(3);
						col2.innerHTML=spearmans[i-1][j-1].toFixed(3);
					}
				}
			}
		},
...
function getMultiData() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			var date = [];
			var names = [];
			var datatemp=data[0].values[0];
			for(var key in datatemp) names.push(key);		
			 var seriesOptions = [];
			 var y=[];
		        seriesCounter = 0,
		        ...
		        $.each(names, function (i, name) {
		    	    ...
		    	    $.each(data, function(n, value) {
					    var dataSet = value.values[0];			
					    temp.push(Date.parse(value.timestamp));
					    temp.push(parseFloat(dataSet[names[i]]));
					    temp2.push(temp);
					    temp = [];
		    	    });
		            seriesOptions[i] = {
